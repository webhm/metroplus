...
const DetalleFiltro = {
    ...
    fetch: () => {
        ...
        DetalleFiltro.data = []
        for (var i = 0; i < BSPedidos.dataFiltros.length; i++) {
            if (BSPedidos.dataFiltros[i].id == BSPedidos.idFiltro) {
                DetalleFiltro.data = BSPedidos.dataFiltros[i];
            }
        }
    ...
    view: () => {
        ...
        if (BSPedidos.showBitacora.length !== 0 && DetalleFiltro.data.length !== 0) {
            return [
            ...
            if (BSPedidos.showBitacora.length !== 0) {
                return [
                    m("div.pd-t-10", [
...
const BSPedidos = {
    ...
    fetch: () => {
        m.request({
            ...
            .then(function (result) {
                BSPedidos.dataFiltros = result.data;
                loadBSPedidos();
    ...
    oninit: (_data) => {
        HeaderPrivate.page = "";
        SidebarLab.page = "";
        BSPedidos.dataPendientesAlta = [];
        BSPedidos.dataCamaTotales = [];
        BSPedidos.camasTotales = 0;
        BSPedidos.pendienteAlta = 0;
    ...
    view: (_data) => {
        ...
        if (_data.attrs !== undefined && _data.attrs.idFiltro !== undefined) {
            ...
            BSPedidos.idFiltro = _data.attrs.idFiltro;
            BSPedidos.showBitacora = "d-none";
            DetalleFiltro.fetch();
                    ...
                    m("div.row.animated.fadeInUp", {
                        ...
                        m("div.col-12", [
                            ...
                            m("div.table-content.col-12.pd-r-0.pd-l-0.pd-b-20.mg-t-70", [
                                ...
                                m("table.table.table-sm.tx-12[id='table-BSPedidos'][width='100%']"),
                            ])
                        ])
                    ]),
                    m(DetalleFiltro)
...
function loadBSPedidos() {
    ...
    var table = $("#table-BSPedidos").DataTable({
        data: BSPedidos.dataFiltros,
        dom: 'ltp',
        language: {
