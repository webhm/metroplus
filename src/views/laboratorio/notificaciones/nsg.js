...
const DetalleFiltro = {
    ...
    fetch: () => {
        ...
        DetalleFiltro.data = []
        for (var i = 0; i < NSGPedidos.dataFiltros.length; i++) {
            if (NSGPedidos.dataFiltros[i].id == NSGPedidos.idFiltro) {
                DetalleFiltro.data = NSGPedidos.dataFiltros[i];
            }
        }
    ...
    view: () => {
        ...
        if (NSGPedidos.showBitacora.length !== 0 && DetalleFiltro.data.length !== 0) {
            return [
            ...
            if (NSGPedidos.showBitacora.length !== 0) {
                return [
                    m("div.pd-t-10", [
...
const NSGPedidos = {
    ...
    oninit: (_data) => {
        HeaderPrivate.page = "";
        SidebarLab.page = "";
        NSGPedidos.dataPendientesAlta = [];
        NSGPedidos.dataCamaTotales = [];
        NSGPedidos.camasTotales = 0;
        NSGPedidos.pendienteAlta = 0;
    ...
    view: (_data) => {
        ...
        if (_data.attrs !== undefined && _data.attrs.idFiltro !== undefined) {
            ...
            NSGPedidos.idFiltro = _data.attrs.idFiltro;
            NSGPedidos.showBitacora = "d-none";
            DetalleFiltro.fetch();
                    ...
                    m("div.row.animated.fadeInUp", {
                        ...
                        m("div.col-12", [
                            ...
                            m("div.table-content.col-12.pd-r-0.pd-l-0.pd-b-20.mg-t-70", [
                                ...
                                m("table.table.table-sm.tx-12[id='table-NSGPedidos'][width='100%']"),
                            ])
                        ])
                    ]),
                    m(DetalleFiltro)
...
function loadNSGPedidos() {
    ...
    var table = $("#table-NSGPedidos").DataTable({
        data: NSGPedidos.dataFiltros,
        dom: 'ltp',
        language: {
